...
% Si on a eu false sur un actionJoueur, c'est forcÈment a cause %de personnageValable. Donc :
...
actionIA(LPerso,LPosition,LPerso,LPositionOut,2):-write('action d√©placer'),joueur(2,Tueur,_,_,_),recupPosition(Tueur,LPerso,LPosition,Case),
...
% ----------------------Tolerance ‡ l'erreur---------------------------
%VÈrifie que c'est une case, et qu'elle est non vide
caseValable(Input):-integer(Input),position(Input,_,X),X\=='vide'.



%VÈrifie que c'est un personnage et qu'il est vivant
